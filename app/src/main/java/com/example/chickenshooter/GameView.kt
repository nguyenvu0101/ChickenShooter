...
class GameView(context: Context, private val backgroundId: Int, planeId: Int) :
    ...
    // Audio control bitmaps
    private val musicOnBitmap = BitmapFactory.decodeResource(context.resources, R.drawable.music_turnon)
    private val musicOffBitmap = BitmapFactory.decodeResource(context.resources, R.drawable.music_turnoff)
    private val soundOnBitmap = BitmapFactory.decodeResource(context.resources, R.drawable.sound_on)
    private val soundOffBitmap = BitmapFactory.decodeResource(context.resources, R.drawable.sound_off)

    // Audio control variables
    private var isMusicEnabled = false // Default: music off (show music_turnoff)
    private var isSoundEnabled = false // Default: sound off (show sound_off)
    private var backgroundMusic: MediaPlayer? = null
    private var musicButtonRect: Rect? = null
    private var soundButtonRect: Rect? = null
    ...
    override fun surfaceCreated(holder: SurfaceHolder) {
        // tạo tọa độ giữa màn hình c��ch 30px so với bottom
    ...
    private fun initializeBackgroundMusic() {
        ...
        backgroundMusic = MediaPlayer.create(context, R.raw.music_background)
    ...
    private fun toggleMusic() {
        ...
        if (isMusicEnabled) {
            // Turn on music: show music_turnon.png
            backgroundMusic?.start()
        } else {
            // Turn off music: show music_turnoff.png
    ...
    private fun toggleSoundEffects() {
        ...
        // Sound effects state will be checked when playing sounds
    }
    ...
    fun update() {
        ...
        // nếu end màn thì mở màn m��i hoặc end game
        ...
        missile?.update()
// Khi tên lửa vừa bắt đ��u nổ, xóa sạch quái và trừ máu boss cho mọi level
        ...
        // Xử lý di chuyển mượt mà cho m��y bay
        if (isMoving) {
            ...
            if (distance > movementSpeed) {
                ...
                // Đến gần mục tiêu, đ��t lại vị trí chính xác
        ...
        // Ki��m tra va chạm với biên màn hình và phát âm thanh (trừ biên dưới)
        val borderThreshold = 5f // ngưỡng để tránh phát ��m liên tục
        ...
        // Gi��i hạn player trong màn h����nh
        ...
        // Kiểm tra nếu máy bay vư���t qua 1/2 màn hình và chưa phát warning gần đây
    ...
    override fun draw(canvas: Canvas) {
        ...
        val audioButtonSize = 60
        val audioButtonSpacing = 70
        val audioButtonY = 140f

        // Music button (on the left)
        val musicButtonX = 40f
        val musicBitmap = if (isMusicEnabled) musicOnBitmap else musicOffBitmap
        val scaledMusicBitmap = Bitmap.createScaledBitmap(musicBitmap, audioButtonSize, audioButtonSize, true)
        canvas.drawBitmap(scaledMusicBitmap, musicButtonX, audioButtonY, paint)
        musicButtonRect = Rect(
            musicButtonX.toInt(),
            audioButtonY.toInt(),
            (musicButtonX + audioButtonSize).toInt(),
            (audioButtonY + audioButtonSize).toInt()
        )

        // Sound effects button (to the right of music button)
        val soundButtonX = musicButtonX + audioButtonSpacing
        val soundBitmap = if (isSoundEnabled) soundOnBitmap else soundOffBitmap
        val scaledSoundBitmap = Bitmap.createScaledBitmap(soundBitmap, audioButtonSize, audioButtonSize, true)
        canvas.drawBitmap(scaledSoundBitmap, soundButtonX, audioButtonY, paint)
        soundButtonRect = Rect(
            soundButtonX.toInt(),
            audioButtonY.toInt(),
            (soundButtonX + audioButtonSize).toInt(),
            (audioButtonY + audioButtonSize).toInt()
        )
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        if (event.action == MotionEvent.ACTION_DOWN) {
            ...
            // Music button click
            if (musicButtonRect?.contains(x, y) == true) {
                toggleMusic()
                return true
            }

            // Sound effects button click
            if (soundButtonRect?.contains(x, y) == true) {
                toggleSoundEffects()
                return true
            }
        }
        ...
        if (pauseButtonRect != null && event.action == MotionEvent.ACTION_DOWN) {
            ...
            if (pauseButtonRect!!.contains(x, y)) {
                isPaused = !isPaused
                // Pause/resume background music when game is paused/resumed
                if (isPaused) {
                    backgroundMusic?.pause()
                } else {
                    if (isMusicEnabled) {
        ...
        if (missileBtnRect != null && event.action == MotionEvent.ACTION_DOWN) {
            ...
            if (missileBtnRect!!.contains(x, y)) {
                // ����ảm bảo chỉ kích hoạt nếu chưa có hiệu ứng và đủ mana
        ...
        // N�����u đang pause thì không xử lý gì thêm
